...
import { updateScatterPlot, updateHistogram, updateParallelCoord } from "@/components/update-plots";
...
export function DrawScatterPlot(props:{
    ...
    const { filteredData, dataFilter, variables, setGeometry, setSelectedId } = useDataContext();
    ...
    useEffect(() =>
        updateScatterPlot(
            ...
            setGeometry,
            setSelectedId
        ), [variables, dataFilter]) // for some reason using plotData re-renders on scatter point click 
...
export function DrawHistogram(props:{
    ...
    const variable = props.selector === "#y-histogram" ? variables.yVar : variables.xVar
    const plotData = filteredData.map(d => ({
        id: d.id,
        x: d[variable]
    }))
