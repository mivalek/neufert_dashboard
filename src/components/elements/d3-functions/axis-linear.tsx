...
export const linearScale = (
  ...
  const dims =
    axis === "x" ? [0 + offset, size - offset] : [size - offset, 0 + offset];
  const fun_result = fun(data, (d: { [key: string]: any }) => d[variable]);
  const domainLimits =
    typeof fun_result === "object" ? fun_result : [0, fun_result];
...
export const linearAxis = (
  ...
  const myAxis = axis === "x" ? axisBottom(scale) : axisLeft(scale);
...
export const grid = (
  ...
  g
    ...
    .attr(
      "y2",
      axis === "x" ? height - margin.bottom + margin.top : (d) => scale(d)
    );
