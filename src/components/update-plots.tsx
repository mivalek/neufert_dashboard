...
import { axisLeft, bin, brush, brushX, brushY, extent, max, line, scaleLinear, scalePoint, select, selectAll, range, BaseType } from "d3"
import { regressionLoess } from 'd3-regression'

import { linearAxis, linearScale, grid } from '@/components/elements/d3-functions/axis-linear';
...
export function updateScatterPlot(data: {
    ...
    setIdFun: React.Dispatch<any>
        ...
        const xAxisLayer = g.select("#x-axis") as d3.Selection<SVGElement, {}, HTMLElement, any>
        const yAxisLayer = g.select("#y-axis") as d3.Selection<SVGElement, {}, HTMLElement, any>
        const xGridLayer = g.select("#x-grid") as d3.Selection<SVGElement, {}, HTMLElement, any>
        const yGridLayer = g.select("#y-grid") as d3.Selection<SVGElement, {}, HTMLElement, any>
        ...
        const selectPoints = ({ selection } : any) => {                 
            ...
            points.classed(`selected ${scatterStyles.selected}`, (d) => {
                ...
                const isSelected = xMin <= datum.x && datum.x <= xMax && yMin <= datum.y && datum.y <= yMax
        ...
        const getSelectedIds = ({ selection } : any) => {
            if (!selection) return
            selectedIDs = []
            selectAll("circle.selected").each(d => selectedIDs.push((d as ScatterDatum).id))
        }
        const setZoom = (axis: "x" | "y", {selection} : any) => {
            ...
            const brushExtent = extent(selection.map(origScale.invert)) as unknown as number[]
        ...
        const xBrush =  brushX() // Add the brush feature using the d3.brush function
            .extent([[0,0], [width, (height + margin.top) * .2 - margin.bottom *.8]]) // initialise the brush area: start at 0,0 and finishes at width,height: it means I select the whole graph area
        ...
        const yBrush =  brushY() // Add the brush feature using the d3.brush function
            .extent([[0,0], [(width + margin.right) * .2 - margin.left *.8, height]]) // initialise the brush area: start at 0,0 and finishes at width,height: it means I select the whole graph area
...
props: {
    ...
    const histogram = bin()
    .value(d => d.x)   // I need to give the vector of value
    
    const binThresholds = (scale: d3.ScaleLinear<number, number, never>, nBins: number) => {
    ...
    if (selector === "#y-histogram") {        
        ...
        histogram.domain(yScale.domain() as [number, number])  // then the domain of the graphic
            .thresholds(binThresholds(yScale, 70)); // then the numbers of bins
            
        const bins = histogram(data);

        xScale.domain([0, max(bins, function(d) { return d.length; }) as number]);  
        linearAxis(xAxisLayer, "x", xScale, true, 4)
        
        grid("x", xGridLayer, xScale, width, height, margin, true, 4)
                
        histogramLayer.selectAll("rect")
            .data(bins)
            .join("rect")
                .attr("x", 0)
                .attr("y", -14) // magic number to offset bars vertically
                .attr("transform", function(d) { return `translate(0, ${yScale(d.x0 as number)})`})
        ...
        histogramLayer.selectAll("rect")
            ...
            .transition()
            .duration(1000)
            .attr("height", function(d) { return yScale(d.x0 as number) - yScale(d.x1 as number)})
        ...
        histogram.domain(xScale.domain() as [number, number])  // then the domain of the graphic
            .thresholds(binThresholds(xScale, 70)); // then the numbers of bins
                
        const bins = histogram(data);

        yScale.domain([0, max(bins, function(d) { return d.length; }) as number]);  
        ...
        .join("rect")
            .attr("x", 1)
            .attr("y", 0)
            .attr("transform", function(d) { return `translate(${xScale(d.x0 as number)})`})
            .classed(histStyles.bar, true)
        ...
        histogramLayer.selectAll("rect")
            ...
            .transition()
            .duration(1000)
            .attr("width", function(d) { return xScale(d.x1 as number) - xScale(d.x0 as number) -1})
            .attr("height", function(d) { return yScale(d.length); })
    }    
}

export function updateParallelCoord(data:
    {[key: string]: number }[] & {id: string}[],
...
props: {
    ...
    const yAxesLayer = g.select("#y-axes") as d3.Selection<SVGElement, {}, HTMLElement, any>

    const yAxes: {[key: string]: d3.Selection<SVGElement, {}, HTMLElement, any>} = {}
    ...
    const makePlot = (transition = true) => {
        ...
        const brushes = yAxesLayer.selectAll(".coordinate-axis")
            .data(activeColumns)
            .join("g")
            .append("g")
            .classed("brush", true)
            .call(brushY()
                .extent([[-25, 0], [15, height]])
                .on("brush", brushed)
                .on("end", brushEnd)
            )
        ...
        if (transition) {
            allLines
                .transition()
                .duration(500)
                .attr("d", (d: {[key: string]: number|string}) => line()(activeColumns.map(p => [xScale(p) as number, yScales.get(p)(d[p])])))
        } else {
            allLines
                .attr("d", (d: {[key: string]: number|string}) => line()(activeColumns.map(p => [xScale(p) as number, yScales.get(p)(d[p])])))
        } // else
    ...
    function updateBrushing() {
        ...
        for (const [column, value] of activeBrushes as Map<string, [number, number]>) {
            const extent = value.map(v => yScales.get(column).invert(v))
            filterArray.push(`d.${column} > ${extent[0]} || d.${column} < ${extent[1]}`)
        }
        ...
        lineContainer?.querySelectorAll(`path:not(.${parallelStyles.hidden})`).forEach(l => lineContainer.appendChild(l))
    } // updateBrushing()

    function brushed({ selection }:{selection: any}, column: string, transition = nData < 1000) {        
